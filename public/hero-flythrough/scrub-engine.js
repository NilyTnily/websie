...
function mountScrollWorld(container, config) {
  ...
  // Scroll-stability window: after the target frame stops moving we keep lerping
  // for this long, then snap the video exactly onto the keyframe. The lerp alone
  // only approaches its target asymptotically — great for mid-scroll smoothing,
  // but at rest it would leave the video a few frames short ("in between" frames)
  // forever. The snap guarantees the resting frame is always exact.
  const SETTLE_MS = 200;
  ...
  SECTIONS.forEach((s, i) => {
    const dive = { kind: 'dive', si: i, clip: s.clip, clipM: s.clipMobile, still: s.still, stillM: s.stillMobile,
                   ...
                   peak: (s.copyPeak != null) ? s.copyPeak : 0.5 };
    SEGMENTS.push(dive);
    s._seg = dive;
  ...
  SEGMENTS.forEach(s => {
    ...
    s._lastTarget = undefined; s._settleAt = 0;
  });

  // per-section copy / route / nav
  const copies = [], dots = [];
  SECTIONS.forEach((s, i) => {
    const c = el('article', 'sw-copy' + (s.copyPos === 'top' ? ' sw-copy--top' : '')); c.style.setProperty('--sw-accent', s.accent || '');
    c.innerHTML =
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      else cop = (before || after) ? 0 : smooth(1 - Math.abs(pr - seg.peak) / 0.5);
      const c = copies[i];
      c.style.opacity = cop;
  ...
  function raf() {
    // Coarser seek step on phones = fewer decodes. A second, looser tier
    // applies while a segment is still actively lerping toward its target
    // (i.e. mid-scrub, not yet settled) — cuts the seek/decode rate during
    // the fast part of a jump, which is where frequent seeking is most
    // likely to read as stutter. Once the settle window is reached, the
    // tight threshold takes back over so the resting frame is still exact.
    const epsSettled = isMobile() ? 0.02 : 0.008;
    const epsScrubbing = isMobile() ? 0.05 : 0.02;
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // A clip that just became scrubbable has no lerp history — if a jump (or
      // the page load) has already moved the scroll, snap straight to that frame
      // instead of drifting up from 0 through "in between" frames.
      if (s._lastTarget === undefined) { s._lastTarget = s.target; s.cur = s.target; }
      // Restart the settle clock whenever the target moves (i.e. any scroll).
      if (s.target !== s._lastTarget) { s._lastTarget = s.target; s._settleAt = now + SETTLE_MS; }
      // Scroll still for a full settle window? Stop lerping and land exactly on
      // the keyframe — the lerp alone converges asymptotically, so at rest it
      // would otherwise sit a few frames short of the target indefinitely.
      const isSettled = now >= s._settleAt;
      s.cur = isSettled ? s.target : s.cur + (s.target - s.cur) * (reduce ? 1 : 0.18);
      const dur = s.video.duration || 1;
      const t = clamp(s.cur, 0, 0.999) * dur;
      const eps = isSettled ? epsSettled : epsScrubbing;
...
function injectCSS() {
  ...
  .sw-copy--top{top:clamp(84px,15vh,150px);transform:none;}
  ...
  .sw-btn--primary{color:var(--sw-bg);background:var(--sw-accent);} .sw-btn--primary:hover{transform:translateY(-2px);background:color-mix(in srgb,var(--sw-accent) 88%,#fff);}
  ...
  .sw-route{position:fixed;right:clamp(14px,2.4vw,30px);top:50%;z-index:40;transform:translateY(-50%);display:flex;flex-direction:column;gap:22px;padding:18px 10px;border-radius:999px;background:color-mix(in srgb,var(--sw-bg) 28%,transparent);border:1px solid color-mix(in srgb,var(--sw-ink) 9%,transparent);backdrop-filter:blur(12px) saturate(140%);-webkit-backdrop-filter:blur(12px) saturate(140%);box-shadow:0 8px 28px color-mix(in srgb,var(--sw-bg) 35%,transparent);}
  ...
  .sw-route__label{position:absolute;right:24px;top:50%;transform:translateY(-50%) translateX(6px);white-space:nowrap;font-family:var(--sw-font-body);font-size:11px;font-weight:500;letter-spacing:.22em;text-transform:uppercase;color:var(--sw-ink);text-shadow:0 1px 12px color-mix(in srgb,var(--sw-bg) 90%,transparent);opacity:0;pointer-events:none;transition:opacity .25s,transform .25s;} .sw-route__dot.is-active .sw-route__label{color:var(--sw-accent);}
