...
const TARGETS: Record<string, number> = {
  "tudor": 300,
  "vacheron-constantin": 278,
  "richard-mille": 300,
  "breitling": 300,
  "jaeger-lecoultre": 300,
  "breguet": 300,
  "piaget": 300,
  "chopard": 300,
  "hublot": 300,
  "zenith": 300,
  "iwc-schaffhausen": 300,
  "panerai": 300,
  "rolex": 322,
  "omega": 300,
};
...
async function scrapeBrand(page: Page, brandSlug: string, brandName: string, maxWatches: number): Promise<ScrapedWatch[]> {
  console.log(`\n=== ${brandName} (target ${maxWatches}) ===`);
  ...
  while (hasMore && collected < maxWatches) {
    ...
    try {
      ...
      if (!nextBtn || pageNum > 50) hasMore = false;
...
async function main() {
  ...
  for(const w of all) doneCount.set(w.brandSlug, (doneCount.get(w.brandSlug)||0)+1);

  for (const b of CURRENT_BRANDS) {
    const target = TARGETS[b.slug] || 40;
    const have = doneCount.get(b.slug) || 0;
    if (have >= target) { console.log(`\nSkipping ${b.name}: have ${have}/${target}`); continue; }
    const need = target - have;
    console.log(`\n=== ${b.name}: have ${have}, need ${need} (target ${target}) ===`);
    const ws = await scrapeBrand(page, b.slug, b.name, need);
    for (const w of ws) {
      all.push(w);
    ...
    console.log(`  ${b.name}: added ${ws.length} (total ${all.length})`);
  }
